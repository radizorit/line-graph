...
*/
    csvParse(csvFile, ticker, colorIndex, timeFrameIndex) {
        ...
        testArray.push(newMap, bollingerBandsLower, bollingerBandsUpper, bottomMap, topMap, closeAverage, accelerationMap, percentageChangeMap, vwapMap)
        ...
        // console.log('this is testMap:', testMap)

        this.setState({
            datasets: this.state.datasets.concat(testMap),
            //.concat(testArray2),
        ...
        console.log('this is this.state.datasets:', this.state.datasets)
        // console.log('BEST', this.state.datasets[timeFrameIndex].data[timeFrameIndex].timeFrameIndex)
        console.log(timeFrameIndex, this.state.datasets[0].timeFrameIndex)
        console.log('Filter:', this.state.datasets.filter((dataset) => dataset.timeFrameIndex == timeFrameIndex).filter((dataset) => {
            return dataset.visibility
        }))
        console.log(this.state.datasets.filter(dataset => dataset.timeFrameIndex == timeFrameIndex).filter((dataset) => {
            return dataset.visibility
        }))
        // console.log('this is testarary2:', testArray2)
    ...
    renderChart(timeFrameIndex) {
        ...
        return (
            <div
                ...
                */
                    // TIMEFRAME INDEX FOR THE ARRAY
                    // USE MAP TO GET THE ARRAYS OF DATA TO POPUALTE
                    // .slice(left, right),
                    // .slice(left, right).filter((dataset) => {
                    //     return dataset.visibility
                    // })
                }} options={{
                    responsive: true,
